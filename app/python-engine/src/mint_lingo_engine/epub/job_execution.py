...
from mint_lingo_engine.providers.translation.ollama_discovery import OllamaModelDiscovery
...
class EpubJobExecutor:
    ...
    def invoke(
        ...
        workflow = EpubTranslationWorkflow(
            TranslationService(self._provider_factory(invocation)),
...
class EpubJobDispatcher:
    ...
    def start(self, payload: object) -> Job | JobStartConflict:
        ...
        started = self._runner.start(
            lambda context: self._executor.invoke(invocation, context, self._on_progress)
        )
